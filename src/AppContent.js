...
export default function AppContent() {
  ...
  function handleAddFren(newFriend) {
    setAddFren((friends) => [...friends, newFriend]);
    setAddForm(false);
  }
  ...
  return (
    <div className="all">
      <div className="left">
        <FriendList addFren={addFren} onSelection={handleSelectedFriend} selectedFriend={selectedFriend} />
        <div className="addBtnContainer">
          <button className="addBtn" onClick={handleAddFriend} style={addForm ? { display: "none" } : {}}>
            Add Friend
          </button>
        </div>
...
function Friend({fren, onSelection, selectedFriend }){
  ...
  function handleBalance(bal){
    if(bal===0){
      ...
      return <p className="userDescription" >you owe {fren.name} ${Math.abs(bal)}.</p>
    }else{
      return <p className="userDescription" >{fren.name} owes you ${Math.abs(bal)}.</p>
    }
  }
  ...
  return <div className="eachFriend" style={isSelected?{backgroundColor:"#A9B5DF",color:"black"}:{}}>
...
function FriendForm({addForm, handleAddFriend, handleAddFren}){
  ...
  const [tasbir, setTasbir] = useState("https://i.pravatar.cc/48");
  ...
  function handleSubmit(event) {
    ...
    setNam("");
    setTasbir("https://i.pravatar.cc/48");
  }
